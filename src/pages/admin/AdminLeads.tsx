...
const AdminLeads = () => {
  ...
  // Fetch BDA profiles from 'bdas' table instead of 'profiles'
  useEffect(() => {
    const fetchBdas = async () => {
      const { data, error } = await supabase
        .from('bdas') // Changed from 'profiles' to 'bdas'
        .select('id, name') // Select only required fields; adjust if 'bdas' has different columns
        .order('name', { ascending: true });

      if (error) {
        console.error('Error fetching BDAs:', error.message, error.details, error.hint, error.code);
  ...
  const handleAssignLeads = async () => {
    ...
    const selectedBda = bdaProfiles.find(bda => bda.id === selectedBdaForAssignment);
    ...
    const updates = Array.from(selectedLeads).map(leadId => ({
      ...
      assignedBdaName: selectedBda.name, // Include name for consistency
      ...
      complete: async (results) => {
        try {
          const newLeads: Lead[] = results.data
            .filter((row: any) => row['Lead Name'] && row['Lead Name'].trim() !== '')
  ...
  const handleExportCSV = () => {
    const exportData = filteredLeads.map(lead => ({
      ...
      'Type': lead.type || '',
      'Status': lead.status,
      'Assigned To': bdaProfiles.find(bda => bda.id === lead.assignedBdaId)?.name || 'Unassigned',
        ...
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-3 sm:space-y-0">
            ...
            <div className="flex items-center space-x-2">
              ...
              >
                <Check className="h-4 w--symbols-4 mr-1.5" />
                Assign
              </button>
            ...
            <thead className="bg-slate-50">
              <tr>
                ...
                <th 
                  scope="col" 
                  className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider cursor-pointer"
                  onClick={() => handleSort('assignedBdaId')}
                >
                  <div className="flex items-center">
                    Assigned To
                    <ArrowUpDown className="h-4 w-4 ml-1" />
            ...
            <tbody className="bg-white divide-y divide-slate-200">
              {filteredLeads.length === 0 ? (
                ...
                filteredLeads.map((lead) => {
                  if (!lead.updatedAt) {
                    console.warn('Lead with missing updatedAt:', lead);
                  }
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        {lead.assignedBdaId ? (
                          <div className="text-sm text-slate-800">
                            {bdaProfiles.find(bda => bda.id === lead.assignedBdaId)?.name || 'Unknown'}
                          </div>
                        ) : (
...
// // Define types locally since types.ts doesn't exist
...
// interface Profile {
//   id: string;
//   full_name: string; // Changed to full_name
//   role: string;
// }
...
//   // Fetch all leads from Supabase on component mount
//   useEffect(() => {
//     const fetchLeads = async () => {
//       const { data, error } = await supabase
//         .from('leads')
...
//   // Fetch BDA profiles (role = 'bda') from the profiles table
//   useEffect(() => {
//     const fetchProfiles = async () => {
//       const { data, error } = await supabase
//         .from('profiles')
//         .select('id, name, role') // Changed to full_name
//         .eq('role', 'bda')
//         .order('name', { ascending: true }); // Changed to full_name

//       if (error) {
//         console.error('Error fetching profiles:', error.message, error.details, error.hint, error.code);
//         toast.error('Failed to load BDAs');
//         return;
//       }
...
//   // Filter and sort leads based on search, status, and BDA
//   const filteredLeads = leads
//     .filter(lead => {
//       const matchesSearch = lead.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
//                            (lead.email && lead.email.toLowerCase().includes(searchTerm.toLowerCase())) || 
...
//   // Open the lead modal for editing a lead
//   const handleOpenLeadModal = (lead: Lead) => {
//     setSelectedLead(lead);
//     setIsModalOpen(true);
//   };
  
//   // Assign selected leads to a BDA
//   const handleAssignLeads = async () => {
//     if (!selectedBdaForAssignment || selectedLeads.size === 0) {
//       toast.error('Please select a BDA and at least one lead');
//       return;
...
//     setSelectedLeads(new Set());
//     setIsAssigningLeads(false);
//     toast.success(`${selectedLeads.size} leads assigned to ${selectedBda.full_name}`); // Changed to full_name
//   };
...
//       'Type': lead.type || '',
//       'Status': lead.status,
//       'Assigned To': bdaProfiles.find(bda => bda.id === lead.assignedBdaId)?.full_name || 'Unassigned', // Changed to full_name
//       'Follow-up Date': lead.followUpDate || '',
//       'Temperature': lead.temperature,
//       'Interests': lead.interests.join(', '),
//       'Remarks': lead.remarks,
...
//   // Save a lead after editing in the modal
//   const handleSaveLead = async (updatedLead: Lead) => {
//     const leadToSave = {
//       ...updatedLead,
//       updatedAt: new Date().toISOString(),
...
//                 <option value="">Select BDA</option>
//                 {bdaProfiles.map((bda) => (
//                   <option key={bda.id} value={bda.id}>{bda.full_name}</option> // Changed to full_name
//                 ))}
//               </select>
...
//                         {lead.assignedBdaId ? (
//                           <div className="text-sm text-slate-800">
//                             {bdaProfiles.find(bda => bda.id === lead.assignedBdaId)?.full_name || 'Unknown'} // Changed to full_name
//                           </div>
//                         ) : (
//                           <div className="text-sm text-slate-500">Unassigned</div>
//                         )}
